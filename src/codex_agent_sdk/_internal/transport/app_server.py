"""Subprocess transport implementation using Codex app-server."""
...
_DEFAULT_MAX_BUFFER_SIZE = 1024 * 1024
...
class AppServerTransport(Transport):
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            not_found_error = CLINotFoundError(
                f"Codex CLI not found at: {self._cli_path}"
            )
            self._exit_error = not_found_error
            raise not_found_error from e
        except Exception as e:
            connection_error = CLIConnectionError(
                f"Failed to start Codex app-server: {e}"
            )
            self._exit_error = connection_error
    ...
    async def end_input(self) -> None:
        # app-server keeps stdin open; no-op
        return None
